...
public class PropertiesPanelController implements IPanel
{
    ...
    @FXML private VBox parentFunction;
...
//    @FXML private Button buttonColourCreate;
    ...
    @FXML private Label labelFunction;
    
    @FXML private Parent parentFunctionType;
    @FXML private ChoiceBox choiceFunctionType;
    
    @FXML private Parent parentImageFunction;
    @FXML private SwingNode imageFunction;
    
    @FXML private TextArea inputFunction;
    
    @FXML private Parent parentFunctionInsert;
    ...
    public void setElement(IDataElement element, ViewType viewType) {
        ...
        if (element != null) {
            switch (element.getType()) {
                case ARC:
                    propertiesPanel.getChildren().add(parentColor);
                    setFunctionFrame(viewType);
                    setArc((DataArc) element);
                    break;

                case PLACE:
                    propertiesPanel.getChildren().add(parentColor);
                    propertiesPanel.getChildren().add(parentToken);
    ...
    private void setFunctionFrame(ViewType viewType) {
        
        parentFunction.getChildren().clear();
        parentFunction.getChildren().add(labelFunction);
                
        switch (viewType) {
            
            case GRAPH:
                parentFunction.getChildren().add(inputFunction);
                break;
                
            case INSPECTOR:
                parentFunction.getChildren().add(parentImageFunction);
                parentFunction.getChildren().add(inputFunction);
                parentFunction.getChildren().add(parentFunctionInsert);
                break;
        }
        
        propertiesPanel.getChildren().add(parentFunction);
    }
